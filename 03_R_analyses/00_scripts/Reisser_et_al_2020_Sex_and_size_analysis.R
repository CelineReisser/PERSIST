...
q <- q + xlab("Populations") + ylab("Length (cm)")+ scale_fill_manual(values=couleurs) + theme_gray() + guides(fill=guide_legend(ncol=2))
...
s <- ggplot(Df,aes(x=cut,fill=sex))+ geom_histogram(stat="count") + facet_grid(stock ~ .) + xlab("Length range (cm)") + scale_fill_manual(values=c("brown","grey28","dodgerblue"),labels=c("Female","Indetermined","Male"))
s <- s +theme_gray(base_size = 18)
s

#see sex ratio per site:
Df<-data
t <- ggplot(Df,aes(x=site,fill=sex))+ geom_histogram(stat="count") + facet_grid("stock", scales = "free") + xlab("Populations") + scale_fill_manual(values=c("brown","grey28","dodgerblue"),labels=c("Female","Indetermined","Male"))
